...
static void btn_click_event_cb(lv_event_t *event)
{
    ...
    if ((int32_t)idx == 1)
    {
        ...
        bt_api_connect(p_bt_dev_info->mac);
        is_connecting = true;
    }
    else
    {
 ...
 *****************************************************************************************
 */
lv_obj_t *lv_bt_pair_dev_layout_create(lv_obj_t *parent_tv_obj, void *user_data)
{
    ...
    char text[BT_DEV_NAME_MAX_LEN + 18] = "";
    sprintf(text, "Pair with device %s?", p_bt_dev_info->dev_name);
