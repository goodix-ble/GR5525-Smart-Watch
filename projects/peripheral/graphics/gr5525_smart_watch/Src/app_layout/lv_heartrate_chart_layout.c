...
#if USE_GX_CHART > 0
static lv_gx_chart_series_t *_hr_ser_max = NULL;
static lv_gx_chart_series_t *_hr_ser_min = NULL;
#else
...
static const lv_style_const_prop_t CHART_STYLE_PROPS[] = {
    ...
    LV_STYLE_CONST_PAD_RIGHT(CHART_PAD_VAL),
#if USE_GX_CHART > 0
    LV_STYLE_CONST_BG_IMG_SRC(&wd_img_heartrate_background),
#endif
    LV_STYLE_PROP_INV,
};
...
static const lv_style_const_prop_t CHART_INDIC_STYLE_PROPS[] = {
    ...
    LV_STYLE_CONST_WIDTH(CAHRT_INDIC_SIZE),
    LV_STYLE_CONST_HEIGHT(CAHRT_INDIC_SIZE),
    LV_STYLE_CONST_BG_OPA(LV_OPA_COVER),
    LV_STYLE_PROP_INV,
};

LV_STYLE_CONST_INIT(HEARTRATE_CHART_WINDOW_STYLE, WINDOW_STYLE_PROPS);
LV_STYLE_CONST_INIT(CHART_REALTIME_LABEL_STYLE, REALTIME_LABEL_STYLE_PROPS);
LV_STYLE_CONST_INIT(CHART_UNIT_LABEL_STYLE, UNIT_LABEL_STYLE_PROPS);
LV_STYLE_CONST_INIT(CHART_STYLE, CHART_STYLE_PROPS);
LV_STYLE_CONST_INIT(CHART_SERIES_STYLE, CHART_SERIES_STYLE_PROPS);
LV_STYLE_CONST_INIT(CHART_INDIC_STYLE, CHART_INDIC_STYLE_PROPS);
 ...
 *****************************************************************************************
 */
lv_obj_t *lv_heartrate_chart_layout_create(lv_obj_t *parent_tv_obj);
...
{
#if USE_GX_CHART > 0
    lv_gx_chart_set_x_start_point(_hr_chart, _hr_ser_min, 0);
    lv_gx_chart_set_x_start_point(_hr_chart, _hr_ser_max, 0);

    for (uint32_t i = 0; i < hours; i++)
    {
        lv_gx_chart_set_next_value(_hr_chart, _hr_ser_min, _hr_24h_data_min[i]);
        lv_gx_chart_set_next_value(_hr_chart, _hr_ser_max, _hr_24h_data_max[i]);
    }
#else
...
lv_obj_t *lv_heartrate_chart_layout_create(lv_obj_t *parent_tv_obj)
{
    ...
    lv_obj_add_style(p_window, (lv_style_t *)&HEARTRATE_CHART_WINDOW_STYLE, 0);
    ...
    lv_obj_add_style(_label_hr_realtime_val, (lv_style_t *)&CHART_REALTIME_LABEL_STYLE, LV_STATE_DEFAULT);
    ...
    lv_obj_add_style(p_label_hr_unit, (lv_style_t *)&CHART_UNIT_LABEL_STYLE, LV_STATE_DEFAULT);
    lv_obj_set_pos(p_label_hr_unit, 214, 50);

#if USE_GX_CHART > 0
    {
        /*Create a chart*/
        _hr_chart = lv_gx_chart_create(p_window);
        lv_obj_add_style(_hr_chart, (lv_style_t *)&CHART_STYLE, LV_STATE_DEFAULT);
        lv_obj_add_style(_hr_chart, (lv_style_t *)&CHART_SERIES_STYLE, LV_PART_ITEMS);
        lv_obj_add_style(_hr_chart, (lv_style_t *)&CHART_INDIC_STYLE, LV_PART_INDICATOR);
        lv_obj_set_size(_hr_chart, 270, 144);
        lv_obj_set_pos(_hr_chart, 40, 124);
        lv_gx_chart_set_type(_hr_chart, LV_GX_CHART_TYPE_LINE);
        lv_gx_chart_set_range(_hr_chart, LV_GX_CHART_AXIS_SECONDARY_Y, 0, 180);
        lv_gx_chart_set_point_count(_hr_chart, 24);

        /*Add two data series*/
        _hr_ser_min = lv_gx_chart_add_series(_hr_chart, lv_palette_main(LV_PALETTE_RED), LV_GX_CHART_AXIS_SECONDARY_Y);
        _hr_ser_max = lv_gx_chart_add_series(_hr_chart, lv_palette_main(LV_PALETTE_GREEN), LV_GX_CHART_AXIS_SECONDARY_Y);
    }
#else
    { /* Set CHART */
        //_hr_chart_bg_img = lv_img_create(g_card_heartrate_detail_chart_win.window);
        // lv_img_set_src(_hr_chart_bg_img, &wd_img_heartrate_background);
        // lv_obj_set_pos(_hr_chart_bg_img, 45, 140);
