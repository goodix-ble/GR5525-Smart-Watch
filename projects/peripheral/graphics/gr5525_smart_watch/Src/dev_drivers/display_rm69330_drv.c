...
#define SCREEN_TYPE                              RM69330_LCD_RES_454
...
static uint32_t s_cur_resolution = RM69330_LCD_RES_454;
...
static void app_qspi_callback(app_qspi_evt_t *p_evt)
{
    ...
    if((p_evt->type == APP_QSPI_EVT_ASYNC_WR_SCRN_CPLT) || (p_evt->type == APP_QSPI_EVT_ASYNC_WR_SCRN_FAIL) || (p_evt->type == APP_QSPI_EVT_TX_CPLT))
    {
...
static void lcd_write(uint8_t cmd, uint8_t *data, uint32_t data_len)
{
    ...
    // g_master_tdone = 0;
    // ret = app_qspi_dma_command_transmit_async(SCREEN_QSPI_ID, &qspi_cmd, data);
    // if(ret == 0)
    //     while(g_master_tdone == 0);
    // else 
    //     printf("Fail:%d \r\n", ret);
    portDISABLE_INTERRUPTS();
    ret = app_qspi_command_transmit_sync(SCREEN_QSPI_ID, &qspi_cmd, data, 0xFFFFFFFF);
...
static void lcd_ca_ra_set(uint16_t x1, uint16_t x2, uint16_t y1, uint16_t y2)
{
    ...
    if(s_cur_resolution == RM69330_LCD_RES_454) {
        x1 += 12;
        x2 += 12;
    } else if(s_cur_resolution == RM69330_LCD_RES_390) {
        x1 += 6;
        x2 += 6;
    }
...
void qspi_screen_init(app_qspi_id_t id, uint32_t clock_prescaler, app_qspi_evt_handler_t evt_handler) {
    rm69330_lcd_res_e default_res = SCREEN_TYPE;
...
void qspi_screen_set_show_area(uint16_t x1, uint16_t x2, uint16_t y1, uint16_t y2) {
    //240x280
    //lcd_ca_ra_set(108 + x1, 108 + x2, 88 + y1, 88 + y2);
    //360x360
    lcd_ca_ra_set(48 + x1, 48 + x2, 48 + y1, 48 + y2);
}
