...
static void indev_proc_press(_lv_indev_proc_t * proc)
{
    ...
    if(indev_obj_act == NULL) {
        ...
        if (disp->sys_layer != NULL && disp->sys_layer->spec_attr != NULL && disp->sys_layer->spec_attr->child_cnt > 0) {
            indev_obj_act = lv_indev_search_obj(disp->sys_layer, &proc->types.pointer.act_point);
        }
        if (indev_obj_act == NULL && disp->top_layer != NULL && disp->top_layer->spec_attr != NULL && disp->top_layer->spec_attr->child_cnt > 0) {
            indev_obj_act = lv_indev_search_obj(disp->top_layer, &proc->types.pointer.act_point);
        }
        #else
...
void indev_gesture(_lv_indev_proc_t * proc)
{
    ...
    while(gesture_obj && lv_obj_has_flag(gesture_obj, LV_OBJ_FLAG_GESTURE_BUBBLE)) {
        ...
        gesture_obj = gesture_obj->parent;
        #else
        /*optimized*/gesture_obj = lv_obj_get_parent(gesture_obj);
        #endif
    }
