...
typedef struct _lv_obj_t {
    ...
    struct __obj_style_cache {
        struct __style_cache_for_main_part {
                lv_opa_t opa;
                lv_opa_t bg_opa;
                lv_opa_t text_opa;
                lv_base_dir_t base_dir;
                lv_text_align_t text_align;
                lv_text_decor_t text_decor : 3;
                lv_blend_mode_t blend_mode: 3;
                uint8_t clip_corner:1;
                ...
                lv_color_t text_color_filtered;
                lv_color_t bg_color;
        ...
        lv_state_t cached_for_state;    /* record if the state has change */

        uint8_t cache_invalid : 1;       /* set to 1 if style has been changed */
        uint8_t enable_label_dsc : 1;    /* set to 1 if label_dsc used */
    } style_cache;
#endif
} lv_obj_t;
 ...
 */
#if LV_GDX_PATCH_INLINE_FUNC
static inline bool lv_obj_has_flag(const lv_obj_t * obj, lv_obj_flag_t f) {
 ...
 */
#if LV_GDX_PATCH_INLINE_FUNC
static inline bool lv_obj_has_flag_any(const lv_obj_t * obj, lv_obj_flag_t f) {
    //LV_ASSERT_OBJ(obj, MY_CLASS);
