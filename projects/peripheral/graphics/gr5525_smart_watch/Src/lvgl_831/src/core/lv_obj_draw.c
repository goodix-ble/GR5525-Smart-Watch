...
void lv_obj_init_draw_rect_dsc(lv_obj_t * obj, uint32_t part, lv_draw_rect_dsc_t * draw_dsc)
{
    ...
    if (part == LV_PART_MAIN) {
        draw_dsc->radius = obj->style_cache.main_part.radius;
    }
    else
    #endif
    /*cached*/draw_dsc->radius = lv_obj_get_style_radius(obj, part);

    if(draw_dsc->bg_opa != LV_OPA_TRANSP) {
        #if LV_GDX_PATCH_STYLE_CACHE_FOR_OBJ
        if (part == LV_PART_MAIN) {
            draw_dsc->bg_opa = obj->style_cache.main_part.bg_opa;
        }
        else
        #endif
        /*cached*/draw_dsc->bg_opa = lv_obj_get_style_bg_opa(obj, part);
    ...
    #if LV_GDX_PATCH_STYLE_CACHE_FOR_OBJ
    if (part == LV_PART_MAIN) {
        draw_dsc->border_width = obj->style_cache.main_part.border_width;
    }
    else
    #else
    #endif
    /*cached*/draw_dsc->border_width = lv_obj_get_style_border_width(obj, part);
    ...
    if(draw_dsc->bg_img_opa != LV_OPA_TRANSP) {
        #if LV_GDX_PATCH_STYLE_CACHE_FOR_OBJ
        if (part == LV_PART_MAIN) {
            draw_dsc->bg_img_src = obj->style_cache.main_part.bg_img_src;
        }
        else
        #else
        #endif
        /*cached*/draw_dsc->bg_img_src = lv_obj_get_style_bg_img_src(obj, part);
...
#endif
#if LV_GDX_PATCH_REMOVE_BORDER || LV_GDX_PATCH_REMOVE_SHADOW
#else
            draw_dsc->border_opa = (opa * draw_dsc->shadow_opa) >> 8;
#endif
#if (LV_GDX_PATCH_REMOVE_OUTLINE || LV_GDX_PATCH_REMOVE_SHADOW)
#else
...
void lv_obj_init_draw_label_dsc(lv_obj_t * obj, uint32_t part, lv_draw_label_dsc_t * draw_dsc)
{
    ...
    if (part == LV_PART_MAIN)
    {
        if (obj->style_cache.enable_label_dsc == 0)
        {
            obj->style_cache.enable_label_dsc = 1;
            lv_obj_update_style_cache_for_label_dsc_only(obj);
        }
        draw_dsc->opa = obj->style_cache.main_part.text_opa;
        if(draw_dsc->opa <= LV_OPA_MIN) return;
        draw_dsc->color = obj->style_cache.main_part.text_color_filtered;
        draw_dsc->letter_space = obj->style_cache.main_part.text_letter_space;
        draw_dsc->line_space = obj->style_cache.main_part.text_line_space;
        draw_dsc->decor = obj->style_cache.main_part.text_decor;
        draw_dsc->font = obj->style_cache.main_part.text_font;
        draw_dsc->align = obj->style_cache.main_part.text_align;
#if LV_USE_BIDI
        draw_dsc->bidi_dir = obj->style_cache.main_part.base_dir;
#endif
    }
    else
    {
        ...
        /*cached*/lv_opa_t opa = lv_obj_get_style_opa(obj, part);
        if(opa <= LV_OPA_MIN) {
...
#endif
#if LV_USE_BIDI
        /*cached*/draw_dsc->bidi_dir = lv_obj_get_style_base_dir(obj, LV_PART_MAIN);
#endif
    }
