...
void lv_obj_get_content_coords(const lv_obj_t * obj, lv_area_t * area)
{
    ...
    lv_coord_t border_width = 0;
#else
    lv_coord_t border_width = obj->style_cache.main_part.border_width;
#endif

    lv_obj_get_coords(obj, area);
    lv_area_increase(area, -border_width, -border_width);
    area->x1 += obj->style_cache.main_part.pad_left;
    area->x2 -= obj->style_cache.main_part.pad_right;
    area->y1 += obj->style_cache.main_part.pad_top;
    area->y2 -= obj->style_cache.main_part.pad_bottom;
#else
#if LV_GDX_PATCH_REMOVE_BORDER
    lv_coord_t border_width = 0;
#else
    ...
    lv_obj_get_coords(obj, area);
    lv_area_increase(area, -border_width, -border_width);
    /*cached*/area->x1 += lv_obj_get_style_pad_left(obj, LV_PART_MAIN);
    /*cached*/area->x2 -= lv_obj_get_style_pad_right(obj, LV_PART_MAIN);
    /*cached*/area->y1 += lv_obj_get_style_pad_top(obj, LV_PART_MAIN);
    /*cached*/area->y2 -= lv_obj_get_style_pad_bottom(obj, LV_PART_MAIN);
#endif
}
...
bool lv_obj_area_is_visible(const lv_obj_t * obj, lv_area_t * area)
{
    ...
    while(par != NULL) {
        ...
        par = par->parent;
        #else
        /*optimized*/par = lv_obj_get_parent(par);
        #endif
    }
