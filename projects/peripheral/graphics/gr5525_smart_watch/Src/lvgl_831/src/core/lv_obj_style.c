...
__attribute__((section("RAM_CODE"))) static lv_style_res_t get_prop_core(const lv_obj_t * obj, lv_part_t part, lv_style_prop_t prop, lv_style_value_t * v);
#else
...
#else
    // not need to call this, it's redundancy
...
lv_style_value_t lv_obj_get_style_prop(const lv_obj_t * obj, lv_part_t part, lv_style_prop_t prop)
{
    ...
    while(obj) {
        ...
        obj = obj->parent;
        #else
        /*optimized*/obj = lv_obj_get_parent(obj);
        #endif
    }
...
#else
            if(false) res = _LV_STYLE_STATE_CMP_SAME;   /* Just provide the if-condition */
#endif
