...
#define lv_refr_get_top_obj(area_p, obj) lv_refr_get_top_obj_opt(area_p, obj)
#define refr_obj_and_children(draw_ctx, top_obj) refr_obj_and_children_opt(draw_ctx, top_obj)
#else
...
#if LV_GDX_PATCH_SIMPLIFY_DRAW_EVENT
void lv_obj_redraw_ignore(lv_draw_ctx_t * draw_ctx, lv_obj_t * obj)
#else
 ...
 */
__attribute__((unused)) static void lv_refr_join_area(void)
{
    uint32_t join_from;
    uint32_t join_in;
...
static void refr_invalid_areas(void)
{
    ...
    for(i = 0; i < disp_refr->inv_p; i++) {
        ...
        if(disp_refr->inv_area_joined[i] == 0) {
            ...
            if (disp_refr->driver->set_clip_area_cb)
            {
                disp_refr->driver->set_clip_area_cb(disp_refr->driver, &disp_refr->inv_areas[i]);
            }
...
static void refr_area_part(lv_draw_ctx_t * draw_ctx)
{
    ...
    #if LV_GDX_PATCH_IGNORE_TRANPARENT_DISPLAY_BG
    if(top_act_scr == NULL && top_prev_scr == NULL && disp_refr->bg_opa > LV_OPA_MIN) {
    #else
    ...
    #if LV_GDX_PATCH_IGNORE_CHILDLESS_SCREEN_LAYER
    lv_obj_t * top_scr = disp_refr->top_layer;
    lv_obj_t * sys_scr = disp_refr->sys_layer;
    if (top_scr != NULL && top_scr->spec_attr != NULL && top_scr->spec_attr->child_cnt > 0) {
        refr_obj_and_children(draw_ctx, top_scr);
    }
    if (sys_scr != NULL && sys_scr->spec_attr != NULL && sys_scr->spec_attr->child_cnt > 0) {
        refr_obj_and_children(draw_ctx, sys_scr);
    }
    #else
...
#endif // !LV_GDX_PATCH_SIMPLIFY_DRAW_EVENT
