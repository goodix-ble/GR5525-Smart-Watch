...
#if LV_GDX_PATCH_DMA_OPTIM > 0u
LV_ATTRIBUTE_FAST_MEM static lv_res_t decode_simple_rgb565(lv_draw_ctx_t * draw_ctx, const lv_draw_img_dsc_t * draw_dsc,
...
uint8_t lv_img_cf_get_px_size(lv_img_cf_t cf)
{
    ...
    switch(cf) {
        ...
        case LV_IMG_CF_ALPHA_8BIT:
            ...
            break;
#if LV_GDX_PATCH_GX_IMG > 0u
        case LV_IMG_CF_GDX_SIMP_RGB565:
            px_size = 16;
            break;
#endif
        default:
                                                      ...
                                                      const lv_area_t * coords, const void * src)
{
...
#if LV_GDX_PATCH_GX_IMG > 0u
    else if(LV_IMG_CF_GDX_SIMP_RGB565 == cdsc->dec_dsc.header.cf) cf = LV_IMG_CF_GDX_SIMP_RGB565;
#endif
    ...
    if((cf == LV_IMG_CF_GDX_SIMP_RGB565) && cdsc->dec_dsc.img_data)
    {
#if LV_GDX_PATCH_DMA_OPTIM > 0u
        //TODO: decode the simple RGB565 format
        decode_simple_rgb565(draw_ctx, draw_dsc, coords, cdsc->dec_dsc.img_data, cdsc->dec_dsc.header.w, cdsc->dec_dsc.header.h);
        draw_cleanup(cdsc);
        return LV_RES_OK;
#endif // LV_GDX_PATCH_DMA_OPTIM > 0u
    }
    else if (cdsc->dec_dsc.header.cf == LV_IMG_CF_GDX_GXIMG)
    {
        lv_port_gximg_draw(draw_ctx, draw_dsc, coords, cdsc->dec_dsc.src, false, false);
        return LV_RES_OK;
    }
    else if (cdsc->dec_dsc.header.cf == LV_IMG_CF_GDX_GXIMG_BUNDLE)
    {
        lv_port_gximg_draw_bundle(draw_ctx, draw_dsc, coords, cdsc->dec_dsc.src);
        lv_draw_rect_dsc_t rect_dsc;
        lv_memset_00(&rect_dsc, sizeof(rect_dsc));
#if LV_GDX_PATCH_REMOVE_BORDER
#else
        rect_dsc.border_opa = LV_OPA_COVER;
        rect_dsc.border_width = 1;
        rect_dsc.border_color = lv_color_make(255, 0, 0);
#endif
        lv_area_t rect;
        lv_area_copy(&rect, draw_ctx->buf_area);
        rect.x1 += 2;
        rect.y1 += 2;
        rect.x2 -= 2;
        rect.y2 -= 2;
        lv_draw_rect(draw_ctx, &rect_dsc, &rect);
        return LV_RES_OK;
    }
                                                      ...
                                                      const lv_area_t * coords, const void * src, int w, int h)
{
    ...
    uint32_t img_w = safe_clip_area.x2 - safe_clip_area.x1 + 1;
    uint32_t img_h = safe_clip_area.y2 - safe_clip_area.y1 + 1;
    uint32_t img_offset_x = safe_clip_area.x1 - coords->x1;
    uint32_t img_offset_y = safe_clip_area.y1 - coords->y1;

    // 4. transfer partial image to safe_clip_area
    ...
    _dma_fill_image_config_t img_fill_cfg;

    img_fill_cfg.dst_buff_address = (uint32_t)draw_ctx->buf;
    img_fill_cfg.dst_buff_height  = lv_area_get_height(draw_ctx->buf_area);
    img_fill_cfg.dst_buff_width   = lv_area_get_width(draw_ctx->buf_area);
    img_fill_cfg.dst_buff_x       = safe_clip_area.x1 - draw_ctx->buf_area->x1;
    img_fill_cfg.dst_buff_y       = safe_clip_area.y1 - draw_ctx->buf_area->y1;

    img_fill_cfg.src_img_address  = (uint32_t) src;
    img_fill_cfg.src_img_h        = h;
    img_fill_cfg.src_img_w        = w;
    img_fill_cfg.src_img_x        = img_offset_x;
    img_fill_cfg.src_img_x_delta  = img_w;
    img_fill_cfg.src_img_y        = img_offset_y;
    img_fill_cfg.src_img_y_delta  = img_h;

    img_fill_cfg.pixel_depth      = 2;

//    printf("Image(0x%08x) on 0x%08x, dst[w:%d,h:%d,x:%d,y:%d], src[w:%d/%d,h:%d/%d,x:%d,y:%d] \r\n", img_fill_cfg.src_img_address,img_fill_cfg.dst_buff_address,
//                            img_fill_cfg.dst_buff_height, img_fill_cfg.dst_buff_width, img_fill_cfg.dst_buff_x, img_fill_cfg.dst_buff_y,
//                            img_fill_cfg.src_img_x_delta, img_fill_cfg.src_img_w, img_fill_cfg.src_img_y_delta, img_fill_cfg.src_img_h,
//                            img_fill_cfg.src_img_x, img_fill_cfg.src_img_y
//                            );
    if(1)
        union_ok = lv_dma_fill_rect_rgb565_image(&img_fill_cfg);

    return union_ok ? LV_RES_OK : LV_RES_INV;
}
#endif
