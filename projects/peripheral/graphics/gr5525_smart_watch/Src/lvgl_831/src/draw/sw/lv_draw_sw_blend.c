                                              ...
                                              lv_coord_t dest_stride, lv_color_t color, lv_opa_t opa, const lv_opa_t * mask, lv_coord_t mask_stride)
{
    ...
    #if 0
            // TODO: fix
            if((color.full == 0x00) || COLOR_ALMOST_BLACK(color.ch.red, color.ch.green, color.ch.blue)) {
                lv_dma_memset((uint32_t)dest_buf, w, dest_stride, h);
            } else {
                lv_dma_fill_color((uint32_t)dest_buf, w, dest_stride, h, color.full);
            }
    #else
            if((color.full == 0x00) || COLOR_ALMOST_BLACK(color.ch.red, color.ch.green, color.ch.blue)) {
                if(w == dest_stride) {
                    memset(dest_buf, 0x00, DISP_PIXEL_DEPTH * w * h);
                    dest_buf += h*dest_stride;
                } else {
                    for(y = 0; y < h; y++) {
                        memset(dest_buf, 0x00, DISP_PIXEL_DEPTH * w);
...

{
    ...
    if(mask == NULL) {
        if(opa >= LV_OPA_MAX) {
            size_t line_size = w * sizeof(lv_color_t);
            for(y = 0; y < h; y++) {
                lv_memcpy(dest_buf, src_buf, line_size);
                dest_buf += dest_stride;
                src_buf += src_stride;
            }
        }
