...
LV_ATTRIBUTE_FAST_MEM void draw_letter_normal_opt(lv_draw_ctx_t * draw_ctx, const lv_draw_label_dsc_t * dsc,
                       ...
                       uint32_t letter)
{
    ...
    #if LV_GDX_PATCH_GLYPH_IN_LABEL_DSC
    const uint8_t * map_p = dsc->p_glyph_bitmap;
    #else //LV_GDX_PATCH_GLYPH_IN_LABEL_DSC
    ...
    if(g.resolved_font->subpx) {
        ...
        if (dsc->bg_opaque)
        {
            draw_letter_normal_opt(draw_ctx, dsc, &gpos, &g, map_p);
        }
        else
        {
