 ...
 * @return true/false
 */
#if LV_GDX_PATCH_BIDI_ACC
static bool lv_bidi_letter_is_weak(uint32_t letter)
{
    return letter > (0x30 - 1) && letter < (0x39 + 1);;
    // bool ret1 = false;
    // uint32_t i = 0;
    ...
    // } while(weaks[i] != '\0');
    // // verify the logic
    // bool ret2 = letter > (0x30 - 1) && letter < (0x39 + 1);
    // if (ret2 != ret1)
    // {
    //     ret1 = ret2;
    // }
...
static bool lv_bidi_letter_is_neutral(uint32_t letter)
{
    static const char neutrals[126] = {
        [0x20] = 0x20, // " "
        [0x09] = 0x09, // \t
        [0x0A] = 0x0A, // \n
        [0x0D] = 0x0D, // \r
    ...
    return letter < 126 && neutrals[letter] != 0;
}
#else
static bool lv_bidi_letter_is_neutral2(uint32_t letter)
{
    static const uint32_t neutrals[128/32] = {
        0x00002600,
        0xFC00F3FF,
        0x38000001,
        0x38000001,
    ...
    if (letter < 126)
    {
        uint32_t dat_idx = letter >> 5;
        uint32_t bit_idx = letter & 0x1F;
        uint32_t mask = 1 << bit_idx;
        return (neutrals[dat_idx] & mask) != 0;
    }
    return false;
}
#endif
#else // LV_GDX_PATCH_BIDI_ACC
static bool lv_bidi_letter_is_neutral(uint32_t letter)
{
    uint16_t i;
