 ...
 **********************/
#if LV_GDX_PATCH_TIMER_PRIO > 0u
//override this function in porting layer
/*__weak bool lv_timer_is_filtered(lv_timer_t * timer){
    return false;
} unfriendly to gcc */
#endif
/**
...
{
#if LV_GDX_PATCH_TIMER_PRIO > 0u
    extern bool lv_timer_is_filtered(lv_timer_t * timer);
    if(true == lv_timer_is_filtered(timer)) {
        return false;
    }
#endif
