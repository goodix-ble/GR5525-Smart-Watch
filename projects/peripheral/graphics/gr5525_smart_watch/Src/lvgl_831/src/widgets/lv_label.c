...
static void draw_main(lv_event_t * e)
{
...
#if LV_GDX_PATCH_CACHE_LABEL_LINE_INFO > 0u
    label_draw_dsc.fast_line_cache = &label->fast_line_cache;
#endif
    ...
    label_draw_dsc.background_color = lv_obj_get_style_bg_color(obj, LV_PART_MAIN);
#endif
...
#if LV_GDX_PATCH_CACHE_LABEL_LINE_INFO > 0u
void update_line_info(lv_obj_t *obj)
{
    lv_label_t *p_label = (lv_label_t *)obj;
    lv_draw_label_fast_line_cache_t *p_fast_line_cache = &p_label->fast_line_cache;
    p_fast_line_cache->is_need_update = true;
