...
static void lv_roller_label_event(const lv_obj_class_t * class_p, lv_event_t * e)
{
...
#if LV_GDX_PATCH_DISABLE_INHERIT_OF_TEXT_STYLE
    LV_UNUSED(res);
#else
...
#endif // LV_GDX_PATCH_DISABLE_INHERIT_OF_TEXT_STYLE
    ...
    else if (code == LV_EVENT_GET_SELF_SIZE)
    {
        ...
        lv_txt_get_size(&size, ((lv_label_t *)label)->text, font, letter_space, line_space, w, LV_TEXT_FLAG_NONE);
...
static void draw_main(lv_event_t * e)
{
    ...
    /*Post draw when the children are drawn*/
        ...
        if(area_ok) {
            ...
            label_dsc.fast_line_cache = &(((lv_label_t *)label)->fast_line_cache);
            label_dsc.fast_line_cache->is_need_update = true;
...
#if LV_GDX_PATCH_CACHE_LABEL_LINE_INFO
    label_draw_dsc.fast_line_cache = &(((lv_label_t *)label_obj)->fast_line_cache);
    label_draw_dsc.fast_line_cache->is_need_update = true;
#endif // LV_GDX_PATCH_CACHE_LABEL_LINE_INFO
...
#if LV_GDX_PATCH_DISABLE_INHERIT_OF_TEXT_STYLE
    lv_text_align_t align = lv_obj_calculate_style_text_align(obj, LV_PART_MAIN, lv_label_get_text(label));
#else
